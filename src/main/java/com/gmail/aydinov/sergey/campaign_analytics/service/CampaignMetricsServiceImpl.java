...
public class CampaignMetricsServiceImpl implements CampaignMetricsService {
	...
	public List<MetricsByDateDto> getTimeSeries(List<EventType> eventTypes, LocalDate from, LocalDate to) {
		SortedMap<LocalDate, List<Impression>> selectedImpressions = dataStore.getAllImpressions().subMap(from,
				to.plusDays(1));
		...
		for (Entry<LocalDate, List<Impression>> entry : impressionsByDay.entrySet()) {
			...
			int clickCount = dataStore.getEventsOfUidsAndTypes(uids, List.of(EventType.fclick)).size();
	...
	public List<AggregationDto> getAggregationByMmDma(List<EventType> eventTypes, LocalDate from, LocalDate to) {
		SortedMap<LocalDate, List<Impression>> selectedImpressions = dataStore.getAllImpressions().subMap(from,
				to.plusDays(1));
		Map<String, List<Impression>> impressionsByMmDma = selectedImpressions.values().stream().flatMap(List::stream)
		...
		for (Entry<String, List<Impression>> entry : impressionsByMmDma.entrySet()) {
			...
			double ctr = impressionsCount == 0 ? 0.0 : 100.0 * clickCount / impressionsCount;
			double evpm = impressionsCount == 0 ? 0.0 : 1000.0 * eventCount / impressionsCount;
			result.add(new AggregationDto(mmDma, impressionsCount, ctr, evpm));
		}
		return result;
	}
	...
	public List<AggregationDto> getAggregationBySiteId(List<EventType> eventTypes, LocalDate from, LocalDate to) {
		SortedMap<LocalDate, List<Impression>> selectedImpressions = dataStore.getAllImpressions().subMap(from,
				to.plusDays(1));
		Map<String, List<Impression>> impressionsBySiteId = selectedImpressions.values().stream().flatMap(List::stream)
		...
		for (Entry<String, List<Impression>> entry : impressionsBySiteId.entrySet()) {
			...
			double ctr = impressionsCount == 0 ? 0.0 : 100.0 * clickCount / impressionsCount;
			double evpm = impressionsCount == 0 ? 0.0 : 1000.0 * eventCount / impressionsCount;
			result.add(new AggregationDto(siteId, impressionsCount, ctr, evpm));
		}
		return result;
	}
